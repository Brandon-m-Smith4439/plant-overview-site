...
assert.ok(controller.includes("animationFps: 30"), "Auto animation frame limiting is missing.");
...
assert.ok(plant.includes("} else if (!projectedBoxVisible(machine)) return [];"), "Static off-screen machines must be rejected without animation cloning.");
...
assert.ok(plant.includes("staticVisibleDesignComponentsCache") && plant.includes("designAnimationPresenceCache"), "Static custom designs and animation presence must be cached between frames.");
assert.ok(plant.includes("suppressGeometryOutlines") && plant.includes("design.components.length > 4"), "Complex overview machines should omit redundant face-outline submissions outside editing.");
...
assert.ok(controller.includes("sampleCount < 24"), "Auto performance adaptation should respond promptly to sustained slow frames.");
