...
assert.match(studio, /const machine = createPlantMachineFromCurrentDesign\(\);/);
assert.match(studio, /if \(!machine\) return;/);
assert.match(studio, /assignment\.value = machine\.instanceId;/);
assert.match(studio, /return machine;/);
